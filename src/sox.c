...
void setupKeyEvent() {
    ...
    CGKeyCode key = (CGKeyCode) 49; // Defined in HIToolbox/Events.h. 0x31 is spacebar
    
    keyPress = CGEventCreateKeyboardEvent(source, key, TRUE);
    keyRelease = CGEventCreateKeyboardEvent(source, key, FALSE);
}
...
static char const * vu(unsigned channel)
{
  ...
  if(channel == 0 && dB > DB_THRESHHOLD && dB_lock == 0) {
      printf("Enough voice detected!\n");
      sendKeyStroke();
      dB_lock = 1;
  } else {
      dB_lock = 0;
...
typedef enum {Full, Type, Rate, Channels, Samples, Duration_default, Duration_secs,
...
static int soxi1(soxi_t const * type, char const * filename)
{
  ...
  switch (*type) {
    ...
    case Duration_default: if (soxi_total ==-1) printf("%s\n", str_time(secs)); break;
